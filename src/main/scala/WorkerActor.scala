...
class WorkerActor extends Actor {
    ...
    def receive = {
        case updateWeightsBetaLambdaPhi (fileName: String, weightM: Array[Double], betaM: Double) => {
            ...
            val mV = new MasterVariables(betaM,weightM,lambda,phi,noOfExamples)
            WorkerVariables wV  = optimize(D.examples,D.labels, 0.07f,0.00001f,mV) 
            weight = wV.weightsk;
            beta = wV.betak;
            
            lambda = lambda + pho(beta -betaM) 
            for(a<- 0 untill noOfExamples){
                phi(a) = phi(a) + pho(weight(i) - weightM(i))
            }
    ...
    def ReadFile(filename:String) :DataSet={
        var d: Dataset = ReadData.buildVector(fileName,",")
        return d
    }
}
